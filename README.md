# SigGear CAD Models
# SigGear CAD Models

CAD models and mechanical reference resources for SigGear compact precision drive solutions, including robot joint modules, compact cycloidal reducers, 6-42mm planetary gear reducers, micro gear motors, and robotic actuator systems.

This repository is intended for engineers, robot developers, mechanical designers, and automation system integrators who need mechanical reference files for compact drive system evaluation, robot joint layout, actuator integration, and early-stage product design.

## Official Product Documentation

* [SigGear Product Docs](https://siggeardrive.github.io/SigGear-product-docs/)
* [6-42mm Planetary Gear Reducer](https://siggeardrive.github.io/SigGear-product-docs/Applications/6-42mm-planetary-gear-reducer/)
* [CPM100-25 Compact Cycloidal Robotic Joint Module](https://siggeardrive.github.io/SigGear-product-docs/CPM100-25/)
* [CPM80-25 Compact Cycloidal Robotic Joint Module](https://siggeardrive.github.io/SigGear-product-docs/CPM80-25/)
* [SG6010C Compact Precision Drive Solution](https://siggeardrive.github.io/SigGear-product-docs/SG6010C/)
* [SG8021 Precision Drive Solution](https://siggeardrive.github.io/SigGear-product-docs/SG8021/)

## CAD Model Categories

This repository may include CAD resources for:

* Compact cycloidal robotic joint modules
* 6-42mm miniature planetary gear reducers
* Micro gear motors for compact robotics
* Robotic gripper drive modules
* Servo motor planetary gearbox assemblies
* Compact actuator layouts for humanoid robots
* Quadruped robot joint drive references
* Robot arm joint mechanical integration references
* AGV / AMR wheel drive gearbox references

## Typical Applications

SigGear CAD models can be used for early mechanical evaluation in:

* Humanoid robot joints
* Quadruped robot legs
* Robot arm joints
* Robotic grippers and end effectors
* AGV / AMR wheel drive systems
* Medical device motion modules
* Laboratory automation equipment
* Compact automation mechanisms
* Servo and brushless motor drive assemblies

## Related Application Pages

* [Cycloidal Reducer for Humanoid Robot Joints](https://siggeardrive.github.io/SigGear-product-docs/Applications/humanoid-robot-joint-reducer/)
* [Quadruped Robot Joint Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Applications/quadruped-robot-joint-gearbox/)
* [Robot Arm Joint Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Applications/robot-arm-joint-gearbox/)
* [Robot Gripper Gear Motor](https://siggeardrive.github.io/SigGear-product-docs/Applications/robot-gripper-gear-motor/)
* [Servo Motor Planetary Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Applications/servo-motor-planetary-gearbox/)
* [AGV / AMR Wheel Drive Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Applications/agv-amr-wheel-drive-gearbox/)
* [Micro Gear Motor for Medical Devices](https://siggeardrive.github.io/SigGear-product-docs/Applications/medical-device-micro-gear-motor/)

## Selection Guides

Before choosing a CAD model, engineers can review these selection guides:

* [Robot Joint Gearbox Selection Guide](https://siggeardrive.github.io/SigGear-product-docs/Selection-Guides/robot-joint-gearbox-selection-guide/)
* [Micro Gear Motor Selection Guide](https://siggeardrive.github.io/SigGear-product-docs/Selection-Guides/micro-gear-motor-selection-guide/)
* [Planetary Gearbox Selection Guide](https://siggeardrive.github.io/SigGear-product-docs/Selection-Guides/planetary-gearbox-selection-guide/)

## Comparison Resources

* [Cycloidal Reducer vs Harmonic Drive](https://siggeardrive.github.io/SigGear-product-docs/Comparisons/cycloidal-vs-harmonic-drive/)
* [Planetary vs Cycloidal Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Comparisons/planetary-vs-cycloidal-gearbox/)
* [Integrated Robot Joint vs Separate Motor Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Comparisons/integrated-robot-joint-vs-separate-motor-gearbox/)

## Developer Resources

For robot actuator control and integration, see:

* [ROS2 Robot Joint Actuator](https://siggeardrive.github.io/SigGear-product-docs/Developers/ros2-robot-joint-actuator/)
* [CAN Protocol Robot Joint Control](https://siggeardrive.github.io/SigGear-product-docs/Developers/can-protocol-robot-joint-control/)

## Notes

CAD models in this repository are intended for reference, layout evaluation, and integration planning. Final dimensions, mounting details, shaft options, and interface requirements should be confirmed with SigGear before production design.

For custom CAD files, STEP models, drawings, datasheets, samples, or gearbox selection support, please contact SigGear.

## Contact

**Email:** [wangwanrong984@gmail.com](mailto:wangwanrong984@gmail.com)


## SigGear Product Documentation

SigGear provides compact precision drive solutions for robotics, automation, medical devices, laboratory equipment, compact actuators, and intelligent motion systems.

Useful documentation links:

* [SigGear Product Docs](https://siggeardrive.github.io/SigGear-product-docs/)
* [6-42mm Planetary Gear Reducer](https://siggeardrive.github.io/SigGear-product-docs/Applications/6-42mm-planetary-gear-reducer/)
* [Cycloidal Reducer for Humanoid Robot Joints](https://siggeardrive.github.io/SigGear-product-docs/Applications/humanoid-robot-joint-reducer/)
* [Quadruped Robot Joint Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Applications/quadruped-robot-joint-gearbox/)
* [Robot Arm Joint Gearbox](https://siggeardrive.github.io/SigGear-product-docs/Applications/robot-arm-joint-gearbox/)
* [Robot Joint Gearbox Selection Guide](https://siggeardrive.github.io/SigGear-product-docs/Selection-Guides/robot-joint-gearbox-selection-guide/)
* [Planetary Gearbox Selection Guide](https://siggeardrive.github.io/SigGear-product-docs/Selection-Guides/planetary-gearbox-selection-guide/)
* [Cycloidal Reducer vs Harmonic Drive](https://siggeardrive.github.io/SigGear-product-docs/Comparisons/cycloidal-vs-harmonic-drive/)
* [ROS2 Robot Joint Actuator](https://siggeardrive.github.io/SigGear-product-docs/Developers/ros2-robot-joint-actuator/)
* [CAN Protocol Robot Joint Control](https://siggeardrive.github.io/SigGear-product-docs/Developers/can-protocol-robot-joint-control/)

Contact: [wangwanrong984@gmail.com](mailto:wangwanrong984@gmail.com)

Official mechanical resources for SigGear robotic transmission products.

This repository provides:

* Mechanical Drawings
* CAD References
* Product Dimensions
* Product Images
* Future STEP Models
* Future URDF Models

---

# Product Families

## Planetary Robotic Joint Modules

### SG6010C

Compact robotic joint actuator.

Applications:

* Exoskeleton Systems
* Humanoid Robots
* Service Robots

### SG8021

High-performance robotic joint actuator.

Applications:

* Humanoid Robots
* Quadruped Robots
* Industrial Robotics

---

## Cycloidal Robotic Joint Modules

### CPM80-25

Compact cycloidal robotic actuator.

Applications:

* Robot Arms
* Exoskeleton Systems
* Educational Robots

### CPM100-25

High torque cycloidal robotic actuator.

Applications:

* Humanoid Hip Joint
* Humanoid Knee Joint
* Collaborative Robots

---

# Repository Structure

```text
SigGear-cad-models

├── SG6010C
│   ├── images
│   ├── drawings
│   └── README.md
│
├── SG8021
│   ├── images
│   ├── drawings
│   └── README.md
│
├── CPM80-25
│   ├── images
│   ├── drawings
│   └── README.md
│
└── CPM100-25
    ├── images
    ├── drawings
    └── README.md
```

---

# Current Resources

Available:

* Product Photos
* Mechanical Drawings
* Dimension Drawings

Planned:

* STEP Files
* STP Files
* URDF Models
* Xacro Models
* Gazebo Meshes

---

# Download Policy

All files in this repository are provided for evaluation and engineering integration.

For custom CAD requests:

[wwang109@163.com](mailto:wwang109@163.com)

---

# Contact

Website:

https://www.siggear.com

Email:

[wangwanrong@siggear.com](mailto:wangwanrong@siggear.com)

Location:

Dongguan, Guangdong, China
